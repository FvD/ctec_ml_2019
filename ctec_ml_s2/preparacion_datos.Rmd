...
Se puede apreciar que aunque los datos en el eje $x$ varian, la proporción de la escala resultante es muy similar al utilzar los métodos `feature_scaling` y `scale`. La función `feature_scaling` utiliza el método de normalización de **mínimos y máximos** (*min-max normalization*) mientras que `scale`, de acuerdo con la documentación, utiliza una variación de la **normalización de la media** (*Mean normalization*) llamado **estandarización** (*standardization*).
...
bathrooms_media <- mean(casas$bathrooms)
desviacion_estandar <- sd(casas$bathrooms)
casas$bathrooms <- (casas$bathrooms - bathrooms_media) / desviacion_estandar
