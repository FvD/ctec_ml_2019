...
```
* Respuesta 1: La función feature_scaling escala los valores en un rango de 0 a 1


```{r}
# Cálculo de z-score
res_sqft_lot <- scale(bck_sqft_lot)
summary(as.numeric(res_sqft_lot))
```
* Respuesta 2: la funcion scale() ó scale (x,scale = TRUE, center = TRUE), lo que hace es calcular un z-score. Podemos determinar lo siguiente:

|CARACTERÍSTICA| SCALE()                             |       FEATURE_SCALING()
|--------------| ----------------------------------- | -----------------------------------------------------
|Cálculo       | Representa normalización z-score    | Representa normalizacion/escalado mínimos-máximos
|Promedio      | El valor tiende a 0                 | Es mayor a 0
|Rango escalado| Rango mínimo negativo y un rango máximo positivo que puede ser mayor a 1 | Está entre 0 y 1
|Función en R  | scale (data = x, scale=TRUE, center=TRUE) | 
...
# Funcion z_score
z_score <- function(x) {
  x_escalado <- ((x - mean(x)) / sd(x) )
  return(x_escalado)
}
# Variable bathrooms con funcion z_score
